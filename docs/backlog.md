...
- [ ] **Click-test the live facet counts in a browser** — the sidebar is now swapped on every filter change (see activity log 2026-09-09). The server side is verified byte-for-byte against a full page load, but the client-side restoration of focus, `<details>` open/closed and scroll offset was written without a browser available. Check: keyboard space-toggling two facets in a row keeps focus; a facet collapsed by the user stays collapsed after a filter change; scrolling Județ halfway then ticking a county does not jump the list.
