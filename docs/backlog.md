...
- [ ] **Watch the first unattended run through the new gate** — `ops/check-export.py` has been validated against a static export and a deliberately corrupted copy, but has never run *inside* a real pipeline run: the wiring in `ops/run-pipeline.sh` is untested end to end. The failure mode to watch for is a false hard-fail blocking a legitimate deploy (exit 65). Also worth checking on the first few runs: `build_meta_fresh` uses a 6h window while the systemd unit allows `TimeoutStartSec=6h`, so a genuinely long run would have its own export judged stale — if durations come in anywhere near that, raise `--max-age-hours` rather than treating it as a data problem.
