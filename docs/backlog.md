...
- [x] **Pipeline observability + an LLM data-quality / pipeline-health check** — **(1), (2) and (4) done 2026-09-10; (3) the LLM-judge pass is the remaining work, tracked separately below.** `pipeline.py` appends a `kind: "run"` record to `data/pipeline-runs.jsonl` (step timings, exit codes, git SHA, trigger, flags); `ops/check-export.py` appends a `kind: "export-check"` record under the same `run_id` with 33 metrics and 24 assertions, and is wired into `ops/run-pipeline.sh` between `export-sqlite` and the deploy — 8 hard checks abort before the rsync (exit 65), 16 soft ones warn. `/pipeline-check` reads both. The per-step counters in the original design were deliberately skipped in favour of state-derived metrics; LLM cost per run is the one thing genuinely lost, and needs a sentinel line out of `llm-schema.py`. `--strict` is off in cron until the soft thresholds have run history behind them. Original text follows. — the unattended VPS run (`ops/run-pipeline.sh`) reports only a healthcheck ping and per-step stdout; no durable per-run record, no automated judgement on the data. Full watch-list and design in **`docs/pipeline-quality-checks.md`**. In short: (1) a structured per-run log (`data/pipeline-runs.jsonl` / a `pipeline_runs` table) with step timings, exit codes, row deltas and LLM cost/failure tallies; (2) post-export assertions run between `export-sqlite` and the deploy, `--strict` in cron — active count within a band vs the previous run, `judete` == 42, `expires_at` not NULL-spiking, schema/inference coverage not regressing, no single `profession_family` dominating (the `IT` bug), no anomaly-flag step-change; reuse `import_csvs.py::{expiry,judet}_sanity_warnings()` and the `export-to-sqlite.py` floors rather than re-implementing; (3) a slower-cadence LLM-judge pass over a sample of freshly extracted postings scoring fidelity against the source and flagging systematic prompt failures; (4) an agent command (`quality-review` extended, or a `pipeline-check` sibling) over the run log + assertion results. Build order in the doc. Prerequisite for leaving the VPS run unattended for days. See also: [~/gov2/monitorulpreturilor/.claude/commands/pipeline-check.md] for a similar intention

- [x] **`HEALTHCHECK_URL` is still empty in `.env`** — **done 2026-09-10**: a once-a-day hc-ping.com check is configured on the VPS. Note the period is set by the *overnight* gap, not the slot interval: 18:33 → 11:45 is 17h12m, so the 12h/3h originally recommended here would have false-alarmed every morning. One day is right; it reports a missed *day* rather than a missed slot, which is the correct trade since both slots run the same thing. `/fail` still alerts immediately. Original text: `ops/run-pipeline.sh::ping_health()` returns early when it is unset, so the dead-man's switch does not exist — nothing anywhere can report a cron slot that never fired, which is exactly how the live site went five weeks stale.

- [ ] **Verify `MAILTO` actually delivers** — the crontab has `MAILTO=pax@mioritics.ro`, which needs an MTA on the box (`command -v sendmail`). A crontab that mails into a void is indistinguishable from a clean run, and it is currently unverified either way. Either confirm it works or drop the line so nobody relies on it; the healthcheck is the real channel.

- [x] **Install `/etc/logrotate.d/posturi` on the VPS** — done 2026-09-10. Edit the path in `ops/logrotate.posturi` if the checkout ever moves off `~pax/g2-dev/posturi.gov.ro`.

- [ ] **Watch the first unattended run through the new gate** — `ops/check-export.py` has been validated against a static export and a deliberately corrupted copy, and confirmed to run in the VPS environment (2026-09-10: 23/23 by hand on `gov2-1`, Django import working so `import_sanity` really runs). It has never run *inside* a real pipeline run: the wiring in `ops/run-pipeline.sh` is untested end to end. The failure mode to watch for is a false hard-fail blocking a legitimate deploy (exit 65). *Not* a concern, contrary to an earlier note here: `build_meta_fresh`'s 6h window has nothing to do with run duration — `export-sqlite` is the last step and the check runs immediately after, so the age between them is seconds however long the run took. The window only fires when the export step failed or was skipped, which is its purpose.

- [ ] **LLM-judge sampling pass over freshly extracted postings** — part 3 of `docs/pipeline-quality-checks.md`, the one piece of that design still unbuilt. Cost-bounded weekly sample of 15–25 postings extracted in the last run: give the model `body_markdown` + `attachment_text` alongside the produced `schema_json` and ask for a fidelity score, checking the known failure modes (`baseSalary` picking up *taxa de concurs*, `validThrough` not matching the body, `responsibilities` holding generic conditions, `positions[]` missing on a multi-role posting). Blocked on having a provider that is not failing ~35% of calls. Land the verdict in `data/pipeline-runs.jsonl` as a third record kind so `/pipeline-check` picks it up with no changes.

- [ ] **Tighten the soft thresholds in `ops/check-export.py` once there is run history** — they were all picked before a single unattended run had been recorded, so some are certainly wrong. The suspects: `family_concentration` fires at 30% while sănătate already sits at 27.7%; `skill_concentration` at 40%; the 15-point anomaly-flag step change; the 5-point coverage-drop floor, which cannot catch a slide of 4 points per run. Review after ~2 weeks of records, then consider turning on `--strict` in `ops/run-pipeline.sh`. `/pipeline-check` is written to propose values from the recorded metrics.

- [ ] **LLM cost and per-error-class tally per run** — the two counters the state-derived run log genuinely cannot reach (everything else, coverage and shares and row counts, is derivable from the export afterwards). Needs `llm-schema.py` to print one machine-readable summary line — calls, tokens in/out, cached, USD, and the per-error-class tally (the `Expected dict, got str` family) — that `pipeline.py` folds into the step record. Without it there is no way to see the LLM bill trending up, or a provider degrading, from the run log alone.
