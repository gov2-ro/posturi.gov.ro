...
- [ ] **`HEALTHCHECK_URL` is still empty in `.env`** — `ops/run-pipeline.sh::ping_health()` returns early when it is unset, so the dead-man's switch does not exist: nothing anywhere can report a cron slot that never fired, which is exactly how the live site went five weeks stale. Everything else built on 2026-09-10 assumes someone eventually looks at the run log; this is the thing that makes them look. Create a healthchecks.io check (period 12h, grace 3h — the slots are 11:45 and 18:33, so that tolerates one late run and catches a missed day) and put the ping URL in the VPS's `.env`. While there, verify `MAILTO` actually works (`command -v sendmail`): a crontab that mails into a void is indistinguishable from a clean run. Setup steps in `docs/deploy-vps.md` §6 *Alerting*.

- [ ] **Install `/etc/logrotate.d/posturi` on the VPS** — `ops/logrotate.posturi` is committed but not installed, so `logs/pipeline.log` grows without bound. `sudo install -m 644 ops/logrotate.posturi /etc/logrotate.d/posturi`, then `sudo logrotate -d /etc/logrotate.d/posturi` to dry-run it. Edit the path in the file if the checkout ever moves off `~pax/g2-dev/posturi.gov.ro`.
