...
- [ ] **Pipeline observability + an LLM data-quality / pipeline-health check** — the unattended VPS run (`ops/run-pipeline.sh`) currently reports only a healthcheck ping (`/start`, `/fail`, success) and whatever each step prints to `journalctl`. There is no durable, machine-readable record of a run and no automated judgement on whether the data it produced is any good. Two parts:
  - **Structured run logging.** Have `pipeline.py` (or `run-pipeline.sh`) emit a per-run summary artifact — JSON lines or a row appended to `data/pipeline-runs.csv` / a `pipeline_runs` table — capturing per step: name, start/end, exit status, rows in/out, LLM calls + token cost, and the counters each step already prints (new/updated/skipped postings, extraction successes by reason, export floor results). Keep the last N and surface the latest on the site's `/despre` or a `?dev=1` panel so "when did the data last actually change, and by how much" is answerable without SSH.
  - **`quality-check` command / cadence.** `quality_check.py` already computes a scored report and there is a `quality-review` skill for the narrative pass, but neither runs automatically. Wire a lightweight check into the pipeline tail (post-export): threshold-based assertions (active count within a sane band vs. the previous run, schema/inference coverage not regressing, județ table == 42, no spike in anomaly flags, no all-`IT` / all-`altele` classification collapse) that ping `/fail` on breach; and, on a slower cadence (weekly?), an LLM pass over a sample of freshly extracted postings scoring extraction fidelity against the source body and flagging systematic prompt failures. Decide where the LLM verdict lands (log artifact + `journalctl`, optional mail). Prerequisite for trusting the VPS run to go days unattended.
