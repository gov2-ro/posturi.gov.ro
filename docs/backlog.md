...
- [ ] **Survey scanned-PDF frequency** — now that OCR works, run a pass over `data/downloads/*.pdf` to measure how many were previously returning empty text and now get chars via OCR. Helps size the remaining coverage gap.
...
- [ ] **Add `agent de securitate` / `agent securitate` / `ofiter securitate` to FAMILIES["ordine publică"]** — currently returns `altele` from dict (0.0 confidence) because only `"agent paza"` is listed, cascading to LLM fallback. Fix in both `quality_check.py::FAMILIES` and `webapp/apps/jobs/management/commands/infer_postings.py`. Identified 2026-05-27.
- [ ] **`ingrijitor` (bare) misclassifies non-healthcare employers as `sănătate`** — `FAMILIES["sănătate"]` contains `"ingrijitor"` but at school/sports club employers this is a facilities janitor. Consider using `row["Employer Category"]` as a secondary signal, or add `"ingrijitor scoala"` / `"ingrijitor camin"` to `tehnic`. Identified 2026-05-27.
