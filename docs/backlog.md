...
- [ ] **`missing_contact` anomaly: distinguish card-empty from attachment-found** — currently both score the same anomaly flag. A posting where the phone is in the `.docx` attachment (but not the HTML card) is not a data quality problem — it's a CSV extraction gap. The anomaly check in `check_infer` should cross-reference `attachment.text` before flagging.
